module ApplicationHelper
  ...
  def link_to_mobile_add_exercise_session_fields(name, f, association)
    new_object = f.object.class.reflect_on_association(association).klass.new
    new_object.weight_sets.build
    fields = f.fields_for(association, new_object, :child_index => "new_#{association}") do |builder|
    #fields = fields_for(association, new_object, :child_index => "new_#{association}") do |builder|
      #safe_concat(render(association.to_s.singularize + "_fields", :f => builder))
      render("exercise_sessions/form.mobile.haml", :form => builder)
    end
    link_to_function(name, "add_fields(this, \"#{association}\", \"#{escape_javascript(fields)}\")")
  end
  ...
  def link_to_mobile_remove_exercise_session_fields(name, f)
    f.hidden_field(:_destroy, :value => 0) + link_to_function(name, "mobile_remove_exercise_fields(this)")
  end
